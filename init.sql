...
RETURN (SELECT count(*)
        FROM SHOPUNIT U
        WHERE u.parentid = $1) = 0;

ALTER TABLE SHOPUNIT
    ADD CONSTRAINT type_check CHECK (check_parent(SHOPUNIT.parentid) AND
                                     (SHOPUNIT.type = 'CATEGORY' OR check_no_children(SHOPUNIT.id)));

ALTER TABLE SHOPUNIT
    ADD CONSTRAINT price_check CHECK ( (SHOPUNIT.type = 'CATEGORY' AND SHOPUNIT.price IS NULL) OR
                                       (SHOPUNIT.type != 'CATEGORY' AND SHOPUNIT.price IS NOT NULL AND
                                        SHOPUNIT.price >= 0));
